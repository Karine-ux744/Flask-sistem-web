...
class EditarPerfil(FlaskForm):
  nome = StringField("Nome",validators=[Optional(),Length(100)])
  sobrenome = StringField("Sobrenome",validators=[Optional(),Length(100)])
  senha = PasswordField("Senha",validators=[Optional(),validar_senha])
  confimacao_senha = PasswordField("Confirme a senha",validators=[Optional(),EqualTo(senha)])
